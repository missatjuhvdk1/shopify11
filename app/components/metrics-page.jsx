...
const renderCalendarIcon = () => (
  <svg
    style={metricsStyles.dateTriggerIcon}
    viewBox="0 0 20 20"
...
export const useMetricsController = (
  initialMetrics,
  { maxRange = MAX_RANGE_DAYS, fetchPath = "/api/metrics/overview" } = {},
) => {
  const fallbackEnd = normalizeDate(new Date());
  const fallbackStart = (() => {
    const start = new Date(fallbackEnd);
    start.setUTCDate(start.getUTCDate() - (initialMetrics?.period?.days || maxRange - 1));
  ...
  const minSelectableDate = useMemo(() => {
    ...
    min.setUTCDate(min.getUTCDate() - (maxRange - 1));
  ...
  const loadMetrics = async (range) => {
    try {
      ...
      fetcher.load(
        `${fetchPath}?id_token=${encodeURIComponent(token)}&startDate=${encodeURIComponent(
...
const DateRangePicker = ({ controller }) => {
  const {
    togglePicker,
    pickerRef,
    displayRangeLabel,
  ...
  return (
    <div style={metricsStyles.controlItem}>
      <span style={metricsStyles.label}>Datumbereik</span>
      <div style={metricsStyles.dateControl} ref={pickerRef}>
        <button type="button" style={metricsStyles.dateTrigger} onClick={togglePicker}>
          {renderCalendarIcon()}
          <span style={metricsStyles.dateTriggerLabel}>{displayRangeLabel}</span>
        </button>
        {isPickerOpen && (
          <div style={metricsStyles.datePicker}>
            <div style={metricsStyles.calendarHeader}>
              <button
                type="button"
                style={{
                  ...metricsStyles.calendarNavButton,
                  ...(canNavigatePrev ? {} : metricsStyles.calendarNavButtonDisabled),
                }}
                onClick={() => {
              ...
              <span style={metricsStyles.calendarTitle}>{formatMonthTitle(pickerMonth)}</span>
              <button
                type="button"
                style={{
                  ...metricsStyles.calendarNavButton,
                  ...(canNavigateNext ? {} : metricsStyles.calendarNavButtonDisabled),
                }}
                onClick={() => {
            ...
            <div style={metricsStyles.weekdayRow}>
              {WEEKDAYS.map((weekday) => (
                <span key={weekday}>{weekday}</span>
              ))}
            </div>
            <div
              style={metricsStyles.calendarGrid}
              onMouseLeave={() => {
              ...
              {calendarDays.map((day, index) => {
                if (!day) {
                  return (
                    <div
                      key={`empty-${index}`}
                      style={{ ...metricsStyles.calendarCell, ...metricsStyles.calendarCellEmpty }}
                    />
                  );
                }

                const dayKey = getDateKey(day);
                const disabled = dayKey < getDateKey(minSelectableDate) || dayKey > getDateKey(today);
                ...
                const cellStyle = {
                  ...metricsStyles.calendarCell,
                  ...(disabled ? metricsStyles.calendarCellDisabled : {}),
                  ...(inRange ? metricsStyles.calendarCellInRange : {}),
                  ...((isStart || isEnd) ? metricsStyles.calendarCellSelected : {}),
                };
            ...
            <div style={metricsStyles.calendarFooter}>
              <span style={metricsStyles.calendarFooterText}>{previewRangeLabel}</span>
              <div style={metricsStyles.calendarFooterActions}>
                <button type="button" style={metricsStyles.tertiaryButton} onClick={cancelRange}>
                  Annuleren
                </button>
                <button
                  type="button"
                  style={{
                    ...metricsStyles.primaryButton,
                    ...(applyDisabled ? metricsStyles.primaryButtonDisabled : {}),
                  }}
                  onClick={applyRange}
...
export const MetricsPageLayout = ({ heading, controller, summaryCards, children }) => (
  <s-page heading={heading} secondary-actions="true">
    <div style={metricsStyles.controls}>
      <DateRangePicker controller={controller} />
      <s-button
        onClick={controller.refreshData}
        {...(controller.isRefreshing ? { loading: true } : {})}
      >
        Vernieuwen
      </s-button>
      <span style={metricsStyles.periodMeta}>
        {controller.metrics.period.label} · Bestellingen: {controller.metrics.summary.totalOrders}
      </span>
    </div>

    {Array.isArray(summaryCards) && summaryCards.length > 0 && (
      <div style={metricsStyles.metricGrid}>
        {summaryCards.map((card) => (
          <s-card key={card.id} padding="tight">
            <div style={metricsStyles.metricCard}>
              <span style={metricsStyles.metricLabel}>{card.label}</span>
              <span style={metricsStyles.metricValue}>{card.value}</span>
              <span style={metricsStyles.metricHelp}>{card.helpText}</span>
            </div>
          </s-card>
        ))}
      </div>
